...
local function listenEventItemPickup(_, data)
    ...
    print("ONGRAB what is hand.ItemHeld?", hand.ItemHeld, hand)
...
local function listenEventItemReleased(_, data)
    ...
    -- print("what was hand.ItemHeld?", hand.ItemHeld, hand)
    ...
    for callback, _ in pairs(registered_event_callbacks[data.game_event_name]) do
        ...
        -- local t = vlua.tableadd(data, {item = Player.LastItemDropped, item_class = data.item})
        -- if type(t.item)=="string" then
        --     print("PLAYER RELEASE STRING", t.item, data.item_name, "but is t2?:")
        --     print(type(t2.item), t2.item)
        --     print("Player.LastItemDropped", Player.LastItemDropped)
        --     print("finish in player.lua")
        -- end
        callback(data)
    end
end
